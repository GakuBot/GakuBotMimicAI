...
function determineOpponentColour(noOfWins, noOfLosses, cooldown, opacity){
  ...
  let hueValue = 120 - colourDiff;
  if(hueValue > 360){
    hueValue = hueValue - 360;
  }
  const satValue = cooldown <= 0 ? 70 : 40;
  const lightValue = cooldown <= 0 ? 60 : 40;
...
function Draw() {
  ...
  this.drawPlayersAndObjectiveNoTarget = function(homeX, homeY, playerX, playerY, opponentX, opponentY, playerCooldown, opponentCooldown, noOfWins, noOfLosses){
      ...
      const giantCircleRadius = height/15;
      ...
      for(let opponentFrameCounter = 1; opponentFrameCounter < opponentTrail.length; opponentFrameCounter++){
        canvas.setColor("fill", determineOpponentColour(noOfWins, noOfLosses, opponentCooldown, 0.1*(opponentTrail.length - opponentFrameCounter)/opponentTrail.length)).setColor("stroke", "blue").circle(opponentTrail[opponentFrameCounter][0], opponentTrail[opponentFrameCounter][1],giantCircleRadius, normalLineThickness,true,false);
      }
      canvas.setColor("fill", opponentColor).setColor("stroke", "blue").circle(thirdX, thirdY,giantCircleRadius, normalLineThickness,true,false);
